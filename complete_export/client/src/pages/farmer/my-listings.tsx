...
export default function MyFarmListingsPage() {
  ...
  // Fetch farmer's product listings with their product details
  const { data: listings, isLoading, isError, error, refetch } = useQuery<ListingWithProduct[]>({
    queryKey: ['/api/farmer-products/my-listings'],
    queryFn: async () => {
      const response = await apiRequest("GET", "/api/farmer-products/my-listings?includeProduct=true");
      if (!response.ok) {
  ...
  const deleteMutation = useMutation({
    mutationFn: async (id: number) => {
      await apiRequest("DELETE", `/api/farmer-products/${id}`);
    },
    onSuccess: () => {
      toast({
          ...
          {filteredListings.map((listing) => (
            <Card key={listing.id} className="overflow-hidden">
              ...
              <CardContent className="pb-4">
                ...
                {listing.isOrganic && (
                  <Badge className="bg-green-100 text-green-800 mb-3">Organic</Badge>
                )}
                ...
                <div className="text-xs text-gray-500 space-y-1 mt-3">
                  ...
                  {listing.status === 'rejected' && listing.adminNotes && (
                    <div className="border-l-2 border-red-400 pl-2 my-2">
                      <p className="font-medium text-red-800">Admin Notes:</p>
                      <p className="text-gray-700">{listing.adminNotes}</p>
                    </div>
                  )}
                  <p>Delivery: {listing.selfDelivery ? 'Self Delivery Available' : 'Transport Agent Required'}</p>
