...
import CategorySubcategoriesDirect from "@/components/category-subcategories-direct";
// Keeping the original for reference, but we'll use the Direct version
...
export default function GroceryPage() {
  ...
  // Query to fetch local products created by admin and service providers
  const { 
    data: localProducts, 
    isLoading: isLoadingLocalProducts,
    isError: isErrorLocalProducts
  } = useQuery({
    queryKey: ["/api/admin/local-products"],
    enabled: true,
  });
  
  // Query to fetch approved provider products (public endpoint)
  const { 
    data: providerProducts, 
    isLoading: isLoadingProviderProducts,
    isError: isErrorProviderProducts
  } = useQuery({
    queryKey: ["/api/public/provider-products"],
    enabled: true,
  });
  ...
  const {
    ...
    queryFn: async ({ queryKey }) => {
      ...
      if (activeCategory !== "all" && activeCategory !== "organic") {
        console.log(`🔍 Fetching subcategories for category ID: ${activeCategory}`);
        ...
        console.log(`📊 Final parentCategoryId for API call: ${parentCategoryId} (type: ${typeof parentCategoryId})`);
        console.log(`🌐 Making API request to: /api/grocery/subcategories?parentCategoryId=${parentCategoryId}`);
        ...
        try {
          ...
          if (data && Array.isArray(data)) {
            console.log(`✅ Subcategories fetched for category ${parentCategoryId}: Found ${data.length} items`);
            ...
            if (data.length > 0) {
              ...
              console.log(`📝 Updating all cache entries for subcategories`);
              ...
              console.log(`⚠️ API returned empty array for category ID: ${parentCategoryId}`);
            }
          } else {
            console.log(`⚠️ API response is not an array:`, data);
          }
          
          return data;
        } catch (error) {
          console.error(`❌ Error in subcategories API call:`, error);
          throw error; // Re-throw to let React Query handle it
        }
      }
      console.log(`ℹ️ Not fetching subcategories because activeCategory is: ${activeCategory}`);
      return [];
    },
    ...
    retry: (failureCount, error) => {
      console.log(`🔄 Subcategory fetch retry #${failureCount}, Error:`, error);
      return failureCount < 2; // Retry only once to avoid too many logs
    },
    onSuccess: (data) => {
      console.log(`✅ Query onSuccess: Loaded ${data?.length || 0} subcategories for category ID: ${activeCategory}`);
      if (data && data.length > 0) {
        console.log(`  First subcategory: ${data[0].name} (ID: ${data[0].id})`);
      }
    },
    onError: (error) => {
      console.error(`❌ Query onError: Failed to load subcategories:`, error);
    }
  });
  ...
  useEffect(() => {
    if (activeCategory !== "all" && activeCategory !== "organic") {
      ...
      if (activeCategory === '4' || activeCategory === 4) {
        console.log('⚠️ OIL CATEGORY DETECTED - using special direct fetch mechanism');
        ...
        console.log(`🔍 Standard fetch for subcategories of category ID: ${activeCategory}...`);
        ...
        // Add a timestamp to prevent caching
        const timestamp = new Date().getTime();
        // Use the dedicated endpoint to avoid route conflicts
        fetch(`/api/grocery/subcategories-public?parentCategoryId=${categoryIdForFetch}&_=${timestamp}`, {
          ...
          .then(data => {
            ...
            if (data && Array.isArray(data)) {
              ...
              // CRITICAL: Set this data directly to useState to bypass all caching mechanisms
              console.log(`⚡ Applying direct state update technique to category ${activeCategory}...`);
              setSubcategoriesForDisplay(data);
  ...
  const handleCategorySelect = (categoryId: string) => {
    ...
    if (categoryObj) {
      ...
      if (categoryObj.id === 4 || categoryId === '4') {
        console.log('⚠️ This is Oils category (ID: 4) - special debugging enabled');
      }
      ...
      fetch(`/api/grocery/subcategories-public?parentCategoryId=${categoryObj.id}&_=${timestamp}`)
        .then(response => {
        ...
        .then(data => {
          ...
          // CRITICAL: Set this data directly to useState to bypass all caching mechanisms
          console.log(`⚡ Applying direct state update technique to category ${categoryId}...`);
          setSubcategoriesForDisplay(data);
  ...
  const allProducts = useMemo(() => {
    ...
    const localProductsList = localProducts || [];
    const providerProductsList = providerProducts || [];
    
    // Convert local products to display format
    const convertedLocalProducts = localProductsList.map((product: any) => ({
      ...product,
    ...
    // Convert provider products to display format
    const convertedProviderProducts = providerProductsList.map((product: any) => ({
      ...product,
      category: product.categoryName || product.category,
      subcategory: product.subcategoryName || product.subcategory,
    ...
    return [...adminProducts, ...convertedLocalProducts, ...convertedProviderProducts, ...approvedFarmerProducts];
  }, [products, localProducts, providerProducts, farmerProducts]);
  
  // Loading and error states
  const isLoading = isLoadingProducts || isLoadingCategories || isLoadingFarmerProducts || isLoadingSubcategories || isLoadingLocalProducts || isLoadingProviderProducts;
  const isError = isErrorProducts || isErrorCategories || isErrorFarmerProducts || isErrorSubcategories || isErrorLocalProducts || isErrorProviderProducts;
                ...
                <div className="space-y-6">
                  ...
                  {activeCategory !== "all" && activeCategory !== "organic" && (
                    <div>
                      ...
                      {/* Apply the subcategory rendering logic to ALL categories, not just Oils */}
                      {subcategoriesForDisplay.length > 0 ? (
                        <div className="space-y-1">
                          {subcategoriesForDisplay.map((subcategory) => {
                            console.log(`Rendering subcategory for mobile: ${subcategory.name} (ID: ${subcategory.id})`);
                            
                            // Special fallback handling for Oils category
                            const isOilCategory = (activeCategory === '4' || activeCategory === 4);
                            ...
                            return (
                              <button
                                ...
                                onClick={() => {
                                  console.log(`Selecting subcategory from mobile: ${subcategory.name} (ID: ${subcategory.id})`);
                                ...
                                <div className="h-6 w-6 mr-2 flex items-center justify-center bg-gray-50 rounded">
                                  <img 
                                    ...
                                    onError={(e) => {
                                      ...
                                      // Fallback path for oil images if in oil category
                                      if (isOilCategory) {
                                        const lowerName = subcategory.name.toLowerCase();
                                        if (lowerName.includes('coconut'))
